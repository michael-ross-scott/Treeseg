...
from feature_extractors.canny_edge_detector import canny_edge_detector as ced
...
def edge_detector_transform(image):
    ...
    detector = ced.cannyEdgeDetector(np_image, sigma=1.4, kernel_size=5, lowthreshold=0.09, highthreshold=0.17,
                                     weak_pixel=100)
    edge_detector_image = detector.detect()

    # edge_detector_image = cv2.Canny(np_image,10,255,7,L2gradient=True)
